...
export function QuizDetailPage() {
  ...
  if (!isManager) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">Доступ запрещён</h3>
        <p className="text-gray-600 dark:text-gray-400 mb-4">Эта страница доступна только менеджерам</p>
  ...
  if (!quiz) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">Тест не найден</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
        ...
        {quiz.description && (
          <p className="text-gray-700 dark:text-gray-300 mb-6 whitespace-pre-wrap">{quiz.description}</p>
        )}

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400">Проходной балл</p>
            <p className="text-2xl font-bold text-primary-600 dark:text-dark-400">{quiz.pass_percent}%</p>
          </div>
          {quiz.time_limit_seconds && (
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Ограничение времени</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{Math.floor(quiz.time_limit_seconds / 60)} мин</p>
            </div>
          )}
          {quiz.max_attempts && (
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Макс. попыток</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{quiz.max_attempts}</p>
            </div>
          )}
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400">Создан</p>
            <p className="text-sm font-medium text-gray-900 dark:text-white">{formatDate(quiz.created_at)}</p>
          </div>
        </div>
      </Card>
        ...
        <nav className="flex gap-8">
          <button
            ...
            className={`py-4 px-1 border-b-2 font-medium text-sm ${
              activeTab === 'sections'
                ? 'border-primary-500 dark:border-dark-500 text-primary-600 dark:text-dark-400'
