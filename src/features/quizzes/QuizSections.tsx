      ...
      {sections.length > 0 ? (
        <div className="space-y-4">
          {sections.map((section) => (
            <Card key={section.id} className="p-6 dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="border-t dark:border-gray-700 pt-4">
                  ...
                  <div className="space-y-2">
                    {section.questions.map((question, index) => (
                      <div key={question.id} className="text-sm text-gray-600 dark:text-gray-400 flex items-start gap-2">
                        <span className="text-gray-400 dark:text-gray-500">{index + 1}.</span>
                        <span className="flex-1 dark:text-gray-300">{question.text}</span>
        ...
        <div className="text-center py-12">
          <p className="text-gray-500 dark:text-gray-400">Секций пока нет. Добавьте первую секцию.</p>
        </div>
      )}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="border dark:border-gray-700 rounded-lg p-4 max-h-60 overflow-y-auto space-y-2 dark:bg-gray-800">
              {questions.length > 0 ? (
                questions.map((question) => (
                  <label key={question.id} className="flex items-start gap-2 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer">
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">Тип: {question.type}</p>
                    </div>
                  </label>
                ))
              ) : (
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-2">
                  Нет доступных вопросов. <Link to="/questions" className="text-primary-600 dark:text-primary-400 hover:underline">Создайте вопросы</Link>
                </p>
              )}
            </div>
          </div>
