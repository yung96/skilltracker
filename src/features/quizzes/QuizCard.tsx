...
export function QuizCard({ quiz, onDelete }: QuizCardProps) {
  return (
    <Card hover className="p-6 h-full flex flex-col dark:bg-gray-800 dark:border-gray-700">
      <div className="flex items-start justify-between mb-4">
        <Badge variant={quiz.status === 'published' ? 'success' : 'warning'}>
          {quiz.status === 'published' ? 'Опубликован' : 'Черновик'}
        </Badge>
        <span className="text-sm font-medium text-primary-600 dark:text-dark-300">
          от {quiz.pass_percent}%
        </span>
      </div>

      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 min-h-[3.5rem] leading-snug">
        {quiz.title}
      </h3>
