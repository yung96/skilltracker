...
export function TaskComments({ taskId, comments, onAdd, onUpdate, onDelete, isLoading, canEdit }: TaskCommentsProps) {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Комментарии</h2>
      ...
      {comments.length > 0 ? (
        <div className="space-y-4">
          {comments.map((comment) => (
            <div key={comment.id} className="border-l-4 border-primary-200 dark:border-dark-500 pl-4">
                ...
                {(isManager || comment.user_id === user?.id) && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-primary-600 hover:text-primary-700 dark:text-dark-500 dark:hover:text-dark-300"
                    >
                    ...
                    <button
                      ...
                      className="text-red-600 hover:text-red-700 dark:text-dark-500 dark:hover:text-dark-500"
                    >
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-center py-8">Комментариев пока нет</p>
      )}
    </div>
  );
}